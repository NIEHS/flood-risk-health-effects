...
```{r}
ggcorr(data = fhs_model_df[, 39:54], progress = F)
```

```{r}
(svi_cor <- cor(fhs_model_df[complete.cases(fhs_model_df[, 39:54]), 39:54]))
```
...
```{r}
ggpairs(data = fhs_model_df, columns = 55:60, progress = F)
```
...
```{r}
ggpairs(data = fhs_model_df, columns = 61:62, progress = F)
```
...
```{r}
collin_var_names <- c("pct_fs_risk_2050_500", "avg_risk_score_all", "pct_fs_risk_2050_100", "pct_fs_risk_2020_500", "pct_fs_risk_2020_100", "avg_risk_fsf_2020_500", "pct_fs_risk_2050_5", "avg_risk_score_2_10", "pct_fs_risk_2020_5")
```
