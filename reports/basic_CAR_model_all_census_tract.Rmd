...
```{r}
fl_subset <- beta_inference_df[2:5, ]
```

```{r}
ggplot(fl_subset, aes(x = var_name, y = post_median)) + 
  geom_point() + 
    theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1), axis.title.x = element_blank(), axis.title.y = element_blank(), 
       axis.text=element_text(size=12), 
       plot.margin = margin(5.5, 5.5, 5.5, 20)) + 
  ...
  geom_hline(yintercept = 0, col = "red") +
  ylim(c(-0.12, 0.18)) + 
  scale_x_discrete(labels = c("Flood Risk PC 1", "Flood Risk PC 2", "Flood Risk PC 3", "Flood Risk PC 4"))
```

```{r}
other_subset <- beta_inference_df[6:32, ]
```

```{r}
ggplot(other_subset, aes(x = var_name, y = post_median)) + 
  geom_point() + 
    theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1), axis.title.x = element_blank(), axis.title.y = element_blank(), 
       axis.text=element_text(size=12), 
       plot.margin = margin(5.5, 5.5, 5.5, 10)) +
  geom_errorbar(aes(ymin = post_2.5, ymax = post_97.5, width = 0.4)) + 
  geom_vline(xintercept = c(16.5, 22.5, 26.5), col = "blue") +
  geom_hline(yintercept = 0, col = "red") +
  annotate(geom = "text", x = 8.5, y = 1.5, label = "Social Vulnerability Index",
           col = "blue", size = 4.5) +
  annotate(geom = "text", x = 19.5, y = 1.5, label = "Air Pollution",
