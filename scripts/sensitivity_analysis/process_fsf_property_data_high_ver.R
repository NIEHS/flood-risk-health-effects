...
for (i in 1:nrow(parcel_data_files)) {
  ...
  pct_ff_df <- flood_risk_parcel %>% group_by(tract_fips) %>% summarise(
    pct_fs_risk_2020_5 = mean(!is.na(high_chance_0_2021) & high_chance_0_2021 >= .20), 
    pct_fs_risk_2050_5 = mean(!is.na(high_chance_0_2051) & high_chance_0_2051 >= .20), 
    pct_fs_risk_2020_100 = mean(!is.na(high_chance_0_2021) & high_chance_0_2021 >= .01), 
    pct_fs_risk_2050_100 = mean(!is.na(high_chance_0_2051) & high_chance_0_2051 >= .01),
    pct_fs_risk_2020_500 = mean(!is.na(high_chance_0_2021) & high_chance_0_2021 >= .002), 
    pct_fs_risk_2050_500 = mean(!is.na(high_chance_0_2051) & high_chance_0_2051 >= .002),
    ...
    avg_risk_fsf_2020_100 = mean(floodfactor[!is.na(high_chance_0_2021) & high_chance_0_2021 >= .01]), 
    avg_risk_fsf_2020_500 = mean(floodfactor[!is.na(high_chance_0_2021) & high_chance_0_2021 >= .002]), 
...
saveRDS(extracted_fr, file = here("intermediary_data/sensitivity_analysis/extracted_fr_high_ver.rds"))
