...
flood_ff_groups <- matrix(NA, nrow = nrow(fhs_model_df), ncol = 3)

flood_ff_groups[, 1] <- fhs_model_df$pct_floodfactor2 + fhs_model_df$pct_floodfactor3 + fhs_model_df$pct_floodfactor4
flood_ff_groups[, 2] <- fhs_model_df$pct_floodfactor5 + fhs_model_df$pct_floodfactor6 + fhs_model_df$pct_floodfactor7
flood_ff_groups[, 3] <- fhs_model_df$pct_floodfactor8 + fhs_model_df$pct_floodfactor9 + fhs_model_df$pct_floodfactor10

flood_ff_groups <- data.frame(flood_ff_groups)

names(flood_ff_groups) <- paste0("flood_ff_group", 1:ncol(flood_ff_groups))
...
fhs_model_df <- data.frame(fhs_model_df, flood_ff_groups)

fhs_model_df <- fhs_model_df %>% relocate(starts_with("flood_ff_group"), .after = RPL_THEMES)
...
saveRDS(fhs_model_df, file = here("intermediary_data/sensitivity_analysis/fhs_model_df_ff_grouped.rds"))
